...
class BinaryFloatTranslator(BaseTranslator):
    ...
    def translate_gene(self, gene):
        ...
        Parses the DNA in this manner:
          1. The first bit determines the sign of the integer portion of the result (0=negative, 1=positive)
          ...
          3. The next bit determines the sign of the exponent portion of the result (0=negative, 1=positive)
        ...
        Example:  let DNA="101101", significand_length=3
          1. "1" indicates a positive sign for the integer portion
          2. "011" is converted into the base-10 integer 3, its sign stays positive due to step (1)
          3. "0" indicates a negative sign for the exponent portion
        ...
        """
        mantissa_sign = 1 if gene.dna[0] == '1' else -1
        mantissa = mantissa_sign * int(gene.dna[1:1 + self.significand_length], base=2)
        
        exponent_sign = 1 if gene.dna[1 + self.significand_length] == '1' else -1
